...
def main_app():
        ...
        for loc in lokasi_dict.keys():
            ...
            for i in range(47, -1, -1):
                ...
                dummy_rows.append({
                    ...
                    "color_sensor_pct": random.randint(20, 98),
    ...
    if nav_option == "Titik Lokasi":
        ...
        st.markdown("Selamat datang di **ECO-MIRACLE**. Halaman ini menampilkan seluruh titik instalasi sensor.")
        ...
        with col_home2:
            ...
            for k, v in lokasi_dict.items():
                st.markdown(f'<div class="home-card"><b style="color:#2e7d32;">{k}</b><br><small>{v["desc"]}</small></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="explanation-box"><strong>Keterangan Pertumbuhan:</strong> Suhu optimal 35-40°C. Jika > 40°C, cooling menyala.</div>', unsafe_allow_html=True)
        st.markdown("---")
        ...
        with c2: # BAGIAN YANG TADI ERROR SUDAH DIPERBAIKI INDENTASINYA
            st.markdown(panel_style, unsafe_allow_html=True)
        ...
        with c4:
            st.markdown(panel_style, unsafe_allow_html=True)
            fig_temp = px.line(hist, x="time", y="temp", title="Tren Suhu", markers=True, color_discrete_sequence=['#ff9800'])
        ...
        st.dataframe(df_tampil[['Waktu', 'ph', 'co2', 'temp']].rename(columns={'ph': 'PH', 'co2': 'CO2 (ppm)', 'temp': 'Suhu (°C)'}), use_container_width=True)

    st.markdown("---")
    st.markdown(f"<p style='text-align: center; color: #757575;'>ECO-MIRACLE v2.0 - {now.year}</p>", unsafe_allow_html=True)
